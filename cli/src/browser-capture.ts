...
import { MarkdownConverter } from './shared/markdown-converter-core.js';
...
export class BrowserCapture {
  ...
  private async extractContent(
    ...
    const markdownText = conversionResult.markdown;
    const characterCount = markdownText.length;
    const imageMatches = markdownText.match(/!\[[^\]]*\]\([^)]+\)/g) ?? [];
    const linkMatches = markdownText.match(/\[[^\]]+\]\([^)]+\)/g) ?? [];
    const imageCount = imageMatches.length;
    const linkCount = Math.max(linkMatches.length - imageMatches.length, 0);
