...
var prismweavebundle = (() => {
  ...
  // src/injectable/prismweave-bundle.ts
  var prismweave_bundle_exports = {};
  __export(prismweave_bundle_exports, {
    PrismWeaveInjectableBundle: () => PrismWeaveInjectableBundle,
    default: () => prismweave_bundle_default
  });
...
`;
  ...
  // src/injectable/markdown-converter-injectable.ts
  var InjectableMarkdownConverter = class _InjectableMarkdownConverter {
    constructor() {
    ...
    async convertToAdvancedMarkdown(html, options = {}) {
      ...
      try {
        ...
        return {
          ...
          warnings: [],
          // The MarkdownConverter handles warnings internally
          wordCount,
          readingTime,
          conversionMethod: "injectable-advanced"
        };
...
---`;
    ...
    _extractMetadata() {
      ...
      metadata.url = window.location.href;
      metadata.domain = window.location.hostname;
      metadata.convertedAt = (/* @__PURE__ */ new Date()).toISOString();
      return metadata;
    }
    _extractTitle() {
      const titleSources = [
    ...
    _removeImages(markdown) {
      return markdown.replace(/!\[[^\]]*\]\([^)]*\)/g, "");
    }
    _removeLinks(markdown) {
      return markdown.replace(/\[([^\]]*)\]\([^)]*\)/g, "$1");
    }
    // Static method for easy access
    static async convertHtml(html, options = {}) {
      const converter = new _InjectableMarkdownConverter();
    ...
    static async createMarkdownDocument(title, content, options = { generateFrontmatter: true }) {
      const result = await _InjectableMarkdownConverter.convertHtml(content, options);
...
${result.markdown}`;
  ...
  if (typeof window !== "undefined") {
    ...
    console.log("\u{1F517} PrismWeave Injectable Markdown Converter loaded successfully");
  }

  // src/injectable/prismweave-bundle.ts
  var PrismWeaveInjectableBundle = class _PrismWeaveInjectableBundle {
    constructor() {
      this._isInitialized = false;
    ...
    async processPage(options = {}) {
      ...
      try {
        ...
        const fullDocument = conversionResult.frontmatter ? `${conversionResult.frontmatter}

${conversionResult.markdown}` : conversionResult.markdown;
        return {
    ...
    // Static method for easy access
    static async processCurrentPage(options = {}) {
      const bundle = new _PrismWeaveInjectableBundle();
      return await bundle.processPage(options);
    }
    // Static method for simple content extraction
    static async extractPageContent(options = {}) {
      const bundle = new _PrismWeaveInjectableBundle();
      return await bundle.extractContent(options);
    }
    // Static method for simple markdown conversion
    static async convertHtmlToMarkdown(html, options = {}) {
      const bundle = new _PrismWeaveInjectableBundle();
  ...
  if (typeof window !== "undefined") {
    ...
    window.prismweaveShowToast = (message, type, clickUrl) => {
      const options = {};
      if (type) options.type = type;
      if (clickUrl) options.clickUrl = clickUrl;
      showToast(message, options);
    };
    console.log("\u{1F680} PrismWeave Injectable Bundle loaded successfully");
    console.log(
      "Available methods: prismweaveProcessPage, prismweaveExtractPageContent, prismweaveConvertHtmlToMarkdown, prismweaveShowToast"
    );
  }
  var prismweave_bundle_default = PrismWeaveInjectableBundle;
  return __toCommonJS(prismweave_bundle_exports);
})();
//# sourceMappingURL=prismweave-bundle.js.map
