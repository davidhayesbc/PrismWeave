...
async function buildWeb() {
  ...
  await copyDir(path.join(rootDir, 'website', 'dist'), distWeb);

  // Copy browser extension distribution
  console.log('🔌 Copying browser extension...');
  await copyDir(path.join(rootDir, 'browser-extension', 'dist'), path.join(distWeb, 'extension'));

  // Copy bookmarklet
  const bookmarkletSrc = path.join(
    rootDir,
    'browser-extension',
    'dist-bookmarklet',
    'bookmarklet.js',
  ...
  try {
    await fs.copyFile(bookmarkletSrc, path.join(distWeb, 'bookmarklet.js'));
  } catch (err) {
