...
class UrlTester {
  private converter: UniversalMarkdownConverter;
  ...
  async testUrl(options: ITestOptions): Promise<ITestResult> {
    const startTime = Date.now();
    
    console.log(chalk.blue(`🌐 Fetching URL: ${options.url}`));
    ...
    try {
      ...
      if (options.verbose) {
        console.log(chalk.cyan('📊 Extracted metadata:'), metadata);
      }
  ...
  private extractMetadata(html: string, url: string): any {
    ...
    if (authorElement) {
      metadata.author = authorElement.getAttribute('content')?.trim() ||
    ...
    if (dateElement) {
      metadata.publishedDate = dateElement.getAttribute('content') ||
  ...
  private async saveResults(result: ITestResult, options: ITestOptions): Promise<void> {
    ...
    if (options.format === 'all' || options.format === 'markdown' || !options.format) {
      ...
      console.log(chalk.gray(`💾 Saved Markdown: ${markdownPath}`));
    }
    ...
    console.log(chalk.gray(`💾 Saved metadata: ${metadataPath}`));
  }

  private displayStats(result: ITestResult): void {
...
async function main() {
  try {
    console.log(chalk.green('🚀 PrismWeave URL Tester\n'));
