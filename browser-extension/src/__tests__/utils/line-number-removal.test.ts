...
import { removeLineNumbers } from '../../utils/test-utilities';
...
The script runs 5 times`;
  ...
  test('R.1.8 - Handle concatenated line numbers (Docker blog issue)', () => {
    // This represents the specific issue from the Docker blog where line numbers
    // are concatenated at the beginning: "12345678910111213141516171819202122232425262728293031"
    const concatenatedNumbersCode = `12345678910111213141516171819202122232425262728293031tree -L 2
├── Dockerfile
...
21 directories, 33 files`;
    ...
    const result = removeLineNumbers(concatenatedNumbersCode);

    // Should start with the actual command, not the concatenated numbers
    expect(result).toMatch(/^tree -L 2/);
    expect(result).toContain('├── Dockerfile');
    expect(result).toContain('├── README-model-runner.md');
    expect(result).toContain('21 directories, 33 files');

    // Should NOT contain the long string of concatenated numbers
    expect(result).not.toMatch(/12345678910111213/);
  });

  // Also test a shorter version to ensure the logic works for different lengths
  test('R.1.9 - Handle shorter concatenated line numbers', () => {
    const shortConcatenatedCode = `123456789echo "Hello World"
echo "Second line"
echo "Third line"`;

    const result = removeLineNumbers(shortConcatenatedCode);

    expect(result).toMatch(/^echo "Hello World"/);
    expect(result).toContain('echo "Second line"');
    expect(result).toContain('echo "Third line"');
