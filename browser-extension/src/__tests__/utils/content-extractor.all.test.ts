...
describe('VIII.3 Simon Willison Blog Content Extraction', () => {
  ...
  test('VIII.3.1 - Should correctly extract main content from Simon Willison blog structure', async () => {
    ...
    // Validate content extraction (robust to whitespace and JSDOM quirks)
    const normalizedContent = result.content.replace(/\s+/g, ' ');
    try {
      expect(normalizedContent).toMatch(/LLMs/i);
      expect(normalizedContent).toMatch(/code/i);
      expect(normalizedContent).not.toMatch(/More recent articles/i);
      expect(normalizedContent).toMatch(/Context is king/i);
      // Allow for shorter content in JSDOM, but require some length
      expect(normalizedContent.length).toBeGreaterThan(50);
    } catch (err) {
      // Output debug info on failure
      // eslint-disable-next-line no-console
      console.error('VIII.3.1 content extraction failed. Extracted content:', result.content);
      throw err;
    }
  });
