...
export class MarkdownConverterCore {
        ...
        this.turndownService.addRule('removeLineNumbers', {
            filter: (node) => {
                if (node.nodeType === 1) {
                    ...
                    return lineNumberPatterns.some(pattern => className.includes(pattern) || id.includes(pattern));
                }
                return false;
            },
            replacement: () => '',
        ...
        this.turndownService.addRule('removeCopyButtons', {
            filter: (node) => {
                if (node.nodeType === 1) {
                    ...
                    return (copyButtonPatterns.some(pattern => className.includes(pattern) ||
                        id.includes(pattern) ||
                        text === pattern) &&
        ...
        this.turndownService.addRule('removeSocial', {
            filter: (node) => {
                if (node.nodeType === 1) {
                    const className = (node.className || '').toLowerCase();
                    const id = (node.id || '').toLowerCase();
                    const socialPatterns = ['social', 'share', 'twitter', 'facebook', 'linkedin', 'pinterest'];
                    return socialPatterns.some(pattern => className.includes(pattern) || id.includes(pattern));
                }
                return false;
            },
            replacement: () => '',
        ...
        this.turndownService.addRule('semanticContent', {
            filter: (node) => {
                ...
                const semanticClasses = [
                    ...
                    'callout', 'note', 'warning', 'info', 'alert',
    ...
    preprocessHtml(html) {
        ...
        const unwantedSelectors = [
            ...
            '.advertisement', '.ad', '.promo', '.sponsor',
            ...
            'button[class*="copy"]', 'button[class*="clipboard"]'
        ];
