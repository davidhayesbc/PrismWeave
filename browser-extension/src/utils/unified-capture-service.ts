...
export class UnifiedCaptureService {
  ...
  async captureContent(
    ...
    try {
      logger.group('🚀 Starting unified content capture');
      ...
      logger.info('⚙️ Capture options:', options);

      // Step 1: Detect content type (unless forced)
      logger.info('🔍 Step 1: Detecting content type...');
      let contentTypeInfo: IContentTypeInfo;

      if (options.forceContentType) {
        logger.info('🎯 Content type forced to:', options.forceContentType);
        contentTypeInfo = {
        ...
        logger.info('🕵️ Auto-detecting content type...');
      ...
      if (options.validateSettings) {
        logger.info('🔐 Step 2: Validating settings before capture...');
        await this.validateCaptureSettings();
        logger.info('✅ Settings validation passed');
      } else {
        logger.info('⏭️ Step 2: Skipping settings validation');
      }

      // Step 3: Route to appropriate capture service based on detected type
      logger.info('🎯 Step 3: Routing to appropriate capture service...');
      let result: IUnifiedCaptureResult;

      switch (contentTypeInfo.contentType) {
        case 'pdf':
          logger.info('📄 Routing to PDF capture service...');
  ...
  private async capturePDFContent(
    ...
    try {
      logger.info('📄 Starting PDF content capture process');
      logger.info('📊 PDF Capture Step 1: Preparing PDF capture options');
      ...
      logger.info('⚙️ PDF Capture Step 2: PDF options prepared:', pdfOptions);
      logger.info('🔌 PDF Capture Step 3: Calling PDF capture service...');
      const pdfResult = await this.pdfCaptureService.capturePDF(data, pdfOptions);

      logger.info('✅ PDF Capture Step 4: PDF capture service completed:', {
      ...
      logger.info('🔄 PDF Capture Step 5: Converting PDF result to unified format...');
      ...
      if (pdfResult.error) {
        ...
        logger.error('❌ PDF capture error details:', pdfResult.error);
      }

      if (pdfResult.data) {
        logger.info('📋 PDF Capture Step 6: Processing PDF result data...');
        unifiedResult.data = {
          ...pdfResult.data,
        ...
        logger.info('💾 PDF Capture Step 7: Creating save result for popup compatibility...');
        ...
        logger.info('✅ PDF save result created:', saveResult);
      }
      ...
      logger.info('🎉 PDF Capture Complete - returning unified result:', {
