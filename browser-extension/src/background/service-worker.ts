...
// HTML to Markdown conversion is handled exclusively by content scripts using TurndownService
...
// Use imported types from types/index.ts - these will be compatible with the service worker context
...
async function handleCapturePage(data?: Record<string, unknown>): Promise<ICaptureResult> {
  try {
    ...
    const currentSettings = await settingsManager.getSettings();
    const validation = settingsManager.validateSettings(currentSettings);
    ...
    try {
      ...
      try {
        ...
        // Content script failed - return HTML only for external conversion
        if (!extractionResult?.success || !extractionResult.data?.html) {
          throw new Error('Direct extraction failed to get HTML content');
        }

        // Return raw extraction data - let UI handle markdown conversion
        const { html, title, url, metadata } = extractionResult.data;

        conversionResult = {
          success: true,
          data: {
            // No markdown conversion in service worker - handled by content scripts only
...
function determineFolder(metadata: IDocumentMetadata, settings: Partial<ISettings>): string {
...
// Test markdown conversion functionality - REMOVED
// Markdown conversion now handled exclusively by content scripts using TurndownService

// Line number removal and HTML-to-Markdown conversion - REMOVED
// Service worker no longer performs markdown conversion
// All conversion is handled by content scripts using TurndownService for consistent, high-quality results
