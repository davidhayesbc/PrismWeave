// Environment-agnostic content extraction core shared across PrismWeave runtimes.
// This module intentionally avoids Chrome-specific APIs so it can execute in
// browser extensions, bookmarklets, or server-side DOM environments (e.g. JSDOM,
// Puppeteer). The logic is copied from the browser extension implementation to
// ensure feature parity across hosts.
...
export class ContentExtractionCore {
  private readonly logger = createConsoleLogger('ContentExtractionCore');
  ...
  extractAdvancedMetadata(): Record<string, unknown> {
    ...
    try {
      ...
      // Ignore structured data parsing issues
    }
